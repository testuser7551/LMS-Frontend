...
//     const shareUrl = `${window.location.origin}/cardview?id=${card._id}`;
...
//             <div className="bg-white rounded-xl shadow-lg p-6 w-[90%] max-w-md relative" onClick={(e) => e.stopPropagation()}>
//                 {/* Close */}
//                 <button
//                     onClick={onClose}
...
function ShareModal({ card, onClose }) {
  ...
  // Pick shareUrl from selectedCard (passed from parent)
  const shareUrl =
    card?.shareUrl || `${window.location.origin}/cardview?id=${card._id}`;
  ...
  const handleWhatsAppSend = () => {
    if (!inputValue) return;
    const message =
      card?.shareType === "certificate"
        ? `Check this certificate: ${shareUrl}`
        : `Check this card: ${shareUrl}`;
  ...
  const handleEmailSend = () => {
    ...
    const body =
      card?.shareType === "certificate"
        ? `Here is the certificate link: ${shareUrl}`
        : `Here is the card link: ${shareUrl}`;
          ...
          <div className="mt-6 flex flex-col items-center">
            ...
            <button
              onClick={() => {
                ...
                const downloadLink = document.createElement("a");
                downloadLink.href = pngUrl;
                downloadLink.download = "share-qr.png";
                document.body.appendChild(downloadLink);
                downloadLink.click();
                document.body.removeChild(downloadLink);
              }}
