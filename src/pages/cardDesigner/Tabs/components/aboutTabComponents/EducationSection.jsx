...
import {
  updateEducationMeta, deleteEducation
} from "../../../../../api/carddesign/contentSection";
import { showToast } from "../../../../../components/toast.js";
import { CardContext } from '../../../../../context/CardContext';

function EducationSection({ educationSection, onChange }) {
  ...
  const handleSave = async () => {
    ...
    //  Simple validation
    if (!educationSection?.educationTitle.trim() && educationSection.isEnabled) {
      showToast('Education Title Required', "top-center", 3000, "dark");
      return;
    }
    try {
      //  Proceed with saving
      const payload = {
        educationTitle: educationSection?.educationTitle,
        isEnabled: educationSection?.isEnabled,
        user_id: userCard?.user_id || "",
      }
      const saved = await updateEducationMeta(payload);
      showToast('Education saved successfully!', "top-center", 3000, "dark");
    }
    catch (err) {
      showToast('Failed to save Education Section', "top-center", 3000, "dark");
    }
  }

  return (
    <div className="bg-[var(--color-bgcolor)] p-4 rounded-lg mt-4 ">
      <div className="flex justify-between items-center mb-5">
        ...
        <div>
          ...
          <label className="flex items-center cursor-pointer">
            <input
              type="checkbox"
              checked={educationSection.isEnabled}
              name="isEnabled"
              onChange={(e) => onChange("isEnabled", e.target.checked)}
              className="sr-only"
            />
            <span
              className={`w-10 h-5 flex items-center rounded-full p-1 duration-300 ease-in-out ${educationSection.isEnabled
                ? "bg-[var(--color-btn-primary)]"
                : "bg-[var(--color-btn-secondary)]"
                }`}
            >
              <span
                className={`bg-[var(--color-bgcolor)] w-4 h-4 rounded-full shadow-md transform duration-300 ease-in-out ${educationSection.isEnabled ? "translate-x-5" : ""
                  }`}
              />
            </span>
          </label>
      ...
      {educationSection.isEnabled && (
        ...
        <input
          type="text"
          value={educationSection.educationTitle}
          name="educationTitle"
          onChange={(e) => onChange("educationTitle", e.target.value)}
          placeholder="Title"
      ...
      {educationSection.isEnabled && educationSection.educationData?.length > 0 && (

        <>
          {/* card with education data to display with edit and delete */}
              ...
              .sort((a, b) => {
                ...
                >
                  ...
                  <div className='flex flex-col justify-center flex-wrap w-full h-full flex-1'>
                    <h3 className="font-bold text-lg text-black">{exp.organization}</h3>
                    <p className="text-md text-secondary">{exp.degree}</p>
                  ...
                  <div className="flex">
                    ...
                    <button
                      onClick={async () => {
                        try {
                          await deleteEducation(exp._id, { user_id: userCard?.user_id }); // API call
                          const updated = educationSection.educationData.filter(
                            (item) => item._id !== exp._id
                          );
                          onChange("educationData", updated);
      ...
      {isModalOpen && (
        <EducationModal
          ...
          onSave={(newExp) => {
            if (editingExp) {
              // Update existing
              const updated = educationSection.educationData.map((item) =>
