...
const ThreadList = ({
  ...
  //console.log("ThreadList props:", { threads, currentUser });
  ...
  .filter((thread) => !thread?.isDeleted?.status)
  ...
  .filter((thread) => {
    if (searchQuery) {
      ...
      return (
        thread.title?.toLowerCase().includes(query) ||
        thread.content?.toLowerCase().includes(query) ||
        thread.user?.fname?.toLowerCase().includes(query) ||
        thread.user?.lname?.toLowerCase().includes(query) ||
        thread.tags?.some((tag) => tag.toLowerCase().includes(query))
      );
    }
    return true;
...
{/* onDelete(thread._id) */}
                  <button onClick={()=> setShowConfirm(true)} className="p-2 bg-red-400 rounded-full shadow-md hover:bg-red-500 hover:scale-105 transition-transform duration-200 cursor-pointer">
  ...
  <ConfirmModal
    ...
    onConfirm={() => { onDelete(thread._id); setShowConfirm(false); }}
