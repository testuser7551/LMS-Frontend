...
const Courses = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        if (user?.role === "admin") {
          // 🟩 Admin: fetch courses filtered by schoolId
          //console.log("🟦 Admin: Fetching courses for school:", schoolId);
          const coursesData = await fetchCoursesAPI(schoolId);
          //console.log("✅ Admin courses fetched:", coursesData?.length || 0, "courses");
          ...
          coursesData.forEach((course) => {
            counts[course._id] = course.enrolledCount || 0;
            //console.log(`📊 Course: "${course.title}", School: ${course.schoolId}, Enrolled: ${course.enrolledCount || 0}`);
          });
          setEnrolledCount(counts);

        } else if (user?.role === "instructor" || user?.role === "mentor") {
          // 🟦 Instructor or Mentor: fetch assigned courses
          //console.log("🟧 Instructor/Mentor: Fetching assigned courses");
          const assignedData = await fetchAssignedCoursesAPI(user._id);
          //console.log("✅ Assigned courses fetched:", assignedData?.length || 0, "courses");
          ...
          // 🟨 Student: find mentor ID first
          //console.log("🟨 Student: Finding mentor and courses");
          const data = await getAllStudentAndMentorIds(); // Fetch all mappings
          const studentId = user._id;
          ...
          if (record && record.mentor) {
            ...
            // Fetch mentor's assigned courses
            const assignedData = await fetchAssignedCoursesAPI(mentorId);
            //console.log("✅ Student courses fetched:", assignedData?.length || 0, "courses");
            setAllCourses(assignedData || []);
        ...
        //console.log("🎉 Finished fetching courses successfully");
        //console.log("📦 Total courses in state:", allCourses.length);
    ...
    if (user && schoolId) {
      ...
      console.warn("⚠️ Cannot fetch courses: User or School ID missing");
  ...
  const handleCertificateDownload = async (cert) => {
    ...
    try {
      ...
      console.error("Certificate download failed:", error);
      showToast("Failed to download the certificate.", "top-center", 10000, "dark");
    }
  };
  ...
  const processedCourses = useMemo(() => {
    ...
    const processed = allCourses.map((course) => {
      const totalLessons = course.chapters.reduce(
        (sum, chapter) =>
          sum +
          chapter.lessons.filter((lesson) => lesson.published === "Published")
    ...
    //console.log("✅ Processed courses:", processed.length);
    return processed;
  }, [allCourses]);
  ...
  // Filtering logic per tab
  const filteredCourses = useMemo(() => {
    //console.log("🔍 Filtering courses for tab:", selectedTab);
    //console.log("📊 Total processed courses:", processedCourses.length);
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              School: {schoolId} | Role: {user?.role}
            </p>
          </div>
          {(user?.role === "admin" || user?.role === "instructor") && (
            <div className="flex gap-2 flex-wrap">
